        ...
        <div v-else-if="verses.length > 0" class="pa-4">
          ...
          <v-row class="ma-0 mt-6">
            <v-col cols="12" class="pa-1 mb-2">
              <v-btn @click="copyToClipboard" color="secondary" block variant="outlined" size="large">
                <v-icon left>mdi-content-copy</v-icon>
                Copiar Capítulo
              </v-btn>
            </v-col>
            <v-col cols="12" class="pa-1">
              <v-btn @click="shareVerse" color="info" block variant="outlined" size="large">
...
import { loadBibleChapter, type BibleVerse } from '../data/bible-data'
...
const loadVerses = () => {
  ...
  try {
    const chapterVerses = loadBibleChapter(selectedBook.value, selectedChapter.value)
    verses.value = chapterVerses
    
    if (chapterVerses.length === 1 && chapterVerses[0].text.includes('exemplo')) {
      showMessage('Dados limitados - Adicione mais conteúdo local para esta passagem')
    }
...
const copyToClipboard = async () => {
  ...
  const text = verses.value.map(v => `${v.number}. ${v.text}`).join('\n')
  const reference = `${currentBookName.value} ${selectedChapter.value}`
